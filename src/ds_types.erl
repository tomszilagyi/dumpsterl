...
to_string({record, {RecName, RecSize}}) ->
    io_lib:format("#~s/~B", [RecName, RecSize]);
to_string({tuple, Size}) ->
    io_lib:format("tuple/~B", [Size]);
to_string(Type) ->
    io_lib:format("~p", [Type]).

%% For complex types, this function returns a list of attributes.
%% The list contains {FieldNo, AttributeName} tuples, where
%% FieldNo is an integer and AttributeName is a string.
%% NB. In case of records, the fields are numbered from 2.
attributes({tuple, N}, _Data) ->
    Fields = lists:seq(1, N),
    Attributes = ["field "++integer_to_list(F) || F <- Fields],
    lists:zip(Fields, Attributes);
attributes({record, {_RecName, RecSize}}, {_Stats, Ext}) ->
    Fields = lists:seq(2, RecSize),
    Attributes =
        case Ext of
            [{Attrs,_Locations}|_] -> [atom_to_list(A) || A <- Attrs];
            [] -> ["field "++integer_to_list(F-1) || F <- Fields]
        end,
    lists:zip(Fields, Attributes);
